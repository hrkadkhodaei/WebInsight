...
crawl_initial, crawl_final = 7, 8 # indices in crawl_list, 0..9
...
features = spark.read.json("/data/doina/WebInsight/2020-" + crawl_list[crawl_initial][2] + "/*.gz") \
	...
	.select(
		...
		udf_numInternalOutLinks(col('url'), col('fetch.internalLinks'), col('fetch.externalLinks')).alias('numInternalOutLinks'), 
...
dataset = features \
	...
	.write.json("dataset/numNewOutlinks_dataset-between_"+crawl_list[crawl_initial][2]+"_and_"+crawl_list[crawl_final][2], 
		mode='overwrite')
